...
\begin{tikzpicture}
    ...
    \vspace{1cm}
    \textbf{$b$-couplage:} ensemble d'arêtes vérifiant $\forall n\in N, \space \sharp\{\{n, p\}\space  | \space p \in N\} \space \leq \space b$.
\end{frame}
...
\begin{frame}
    ...
    \item
        ...
        \begin{itemize}
            ...
            \item Première adaptation de l'algorithme pour un réseau dynamique
            \item Deuxième adaptation moins naïve
        \end{itemize}

    \item Adaptation dans un graphe de préférence cyclique
    \item Validation des résultats via temps d'activité des liaisons
    \end{itemize}
\end{frame}
...
\begin{frame}
    \frametitle{Couplage stable statique - Résultats}
        ...
        \end{figure}
    \centering
     Complexité: $O(C_bn^{a})$ où $a \simeq 2$
\end{frame}
...
\begin{frame}
    \frametitle{Couplage stable dynamique naïf - Résultats}
        \begin{figure}
            ...
            \caption{Ajouts successifs de 200 noeuds à partir de $|R|=10$}
        \end{figure}
\end{frame}
...
\begin{frame}
    ...
    Mesure de satisfaction\footnote{\textit{Georgiadis \& Papatriantafilou, 2012}} pour un noeud $i$:
        \begin{equation}
            S_i = \frac{1}{b} . (c_i + \frac{\sum_{j}{R_j - C_j}}{L_j}) \in [0, 1]
        \end{equation}
...
\begin{frame}
    \frametitle{Annexe - Mesure de satisfaction}
    \begin{equation}
        S = \sum_{i}{S_i} = 
        \sum_{i}{ \frac{1}{b} . (c_i + \frac{\sum_{j}{R_j - C_j}}{L_i})} \in [0, 1]
    \end{equation}
